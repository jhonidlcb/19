...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/client/stage-invoices/:stageId/download-resimple", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      // Generate boleta number with sequential format
      const boletaPrefix = company?.boletaPrefix || '001-001';
      const currentSequence = company?.boletaSequence || 1;
      const boletaNumber = `${boletaPrefix}-${String(currentSequence).padStart(7, '0')}`;

      // Update boleta sequence for next invoice
      if (company) {
      ...
      // ==> INFORMACIÓN DE LA FACTURA (con datos de timbrado) <==
      doc.fontSize(9).fillColor('#1e293b');
      doc.text(`Boleta N°: ${boletaNumber}`, leftMargin, yPos);
      doc.text(`Fecha: ${issueDate}`, leftMargin + 180, yPos);
      doc.text(`Etapa: ${stageNumber} de ${totalStages}`, leftMargin + 380, yPos);
      yPos += 12;
      
      // Información de timbrado
      if (company?.timbradoNumber) {
        doc.fontSize(8).fillColor('#475569');
        doc.text(`Timbrado N°: ${company.timbradoNumber}`, leftMargin, yPos);
        if (company?.vigenciaTimbrado && company?.vencimientoTimbrado) {
          doc.text(`Vigencia: ${company.vigenciaTimbrado} - Vto: ${company.vencimientoTimbrado}`, leftMargin + 180, yPos);
        }
        yPos += 12;
      }
      yPos += 8;
      ...
      yPos += 12;
      doc.text(`Ciudad: ${company?.city || 'Carlos Antonio López'} – ${company?.department || 'Itapúa'} – ${company?.country || 'Paraguay'}`, leftMargin, yPos);
      yPos += 12;
      ...
      yPos += 10;
      doc.text(`Código de verificación: RES-${boletaNumber}`, leftMargin, yPos);
      yPos += 20;
